...
export const BookingDetailModal = ({
  ...
  const policyDescription =
    policy?.description ||
    `Cancel ${policy?.deadlineHours || 24} hours before check-in for refund.`;
  const canCancel =
    (booking.status === "confirmed" || booking.status === "pending") &&
    !isNonRefundable;
  let estimatedRefund = 0;
  if (!isNonRefundable && hoursUntilCheckIn >= (policy?.deadlineHours || 0)) {
    estimatedRefund = (totalPrice * (policy?.refundPercentage || 100)) / 100;
  }
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto relative flex flex-col">
          ...
          <div className="p-6 space-y-6">
            ...
            {booking.status !== "canceled" && (
              ...
              >
                <AlertTriangle
                  className={`w-5 h-5 shrink-0 ${!isNonRefundable ? "text-yellow-600" : "text-red-600"}`}
                />
                <div>
                  <h4
                    className={`text-sm font-semibold ${!isNonRefundable ? "text-yellow-800 dark:text-yellow-500" : "text-red-800 dark:text-red-400"}`}
                  >
                    {!isNonRefundable
                      ? "Cancellation Policy"
                      : "Non-Refundable"}
                  ...
                  <p
                    className={`text-xs mt-1 ${!isNonRefundable ? "text-yellow-700 dark:text-yellow-400" : "text-red-700 dark:text-red-300"}`}
                  >
                    {policyDescription}
                  </p>
                </div>
              ...
              {isPriceExpanded && (
                <div className="p-4 bg-white dark:bg-gray-800 space-y-3 text-sm">
                  <div className="flex justify-between text-gray-600 dark:text-gray-400">
                    <span>
                      ${pricePerNight} × {calculatedNights} nights
                    </span>
                    <span>${basePrice}</span>
                  </div>
                  <div className="flex justify-between text-gray-600 dark:text-gray-400">
        ...
        <div className="space-y-4">
          ...
          <div className="text-center">
            ...
            >
              ...
              <p
                className={`text-2xl font-bold ${estimatedRefund > 0 ? "text-green-600" : "text-red-600"}`}
              >
              ...
              {isNonRefundable ? (
                ...
                hoursUntilCheckIn < (policy?.deadlineHours || 0) && (
                  <p className="text-[10px] text-red-400 mt-1 italic">
                    *Past the cancellation deadline ({policy?.deadlineHours}h).
                  </p>
                )
              )}
            </div>
...
const InfoBox = ({
  ...
  <div
    className={`bg-gray-50 dark:bg-gray-700/50 rounded-xl p-3 border border-gray-100 dark:border-gray-700 ${className}`}
  >
