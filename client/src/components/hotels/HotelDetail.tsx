...
export function HotelDetail({ hotel }: HotelDetailProps) {
    ...
    return (
        ...
        <div className="max-w-7xl mx-auto px-6 py-8">
            ...
            {/* Hotel Header */}
            <div className="mb-12">
                <h1 className="text-4xl text-gray-900 dark:text-white mb-4">{hotel.name}</h1>
                ...
                <div className="flex items-center gap-1 mb-4">
                    {Array.from({ length: 5 }).map((_, i) => {
                        ...
                        if (hotel.rating >= starValue) {
                            return <Star key={i} className="w-5 h-5 fill-blue-600 text-blue-600" />;
                        }
                        else if (hotel.rating >= starValue - 0.5) {
                            return <StarHalf key={i} className="w-5 h-5 fill-blue-600 text-blue-600" />;
                        }
                        else {
                    ...
                    <span className="ml-2 text-sm text-gray-600 font-medium">{hotel.rating}</span>
                </div>
                ...
                {/* Hotel Amenities */}
                <div className="mb-6">
                    <h4 className="text-base text-gray-500 dark:text-gray-300 mb-3">Amenities</h4>
            ...
            <div>
                <div className="mb-8">
                    <h2 className="text-3xl text-gray-900 dark:text-white mb-2">Available Rooms</h2>
                    <p className="text-gray-600 dark:text-gray-400">Choose from our selection of luxury rooms and suites</p>
                </div>
                ...
                <div className="space-y-6">
                    ...
                    {rooms.length > 0 && rooms.filter(room => room.maxPeople >= guests).length === 0 && (
                        <div className="p-6 light:bg-yellow-50 text-red-600 rounded-lg text-center">
                            <p className="font-bold">No rooms available for {guests} guests in one room.</p>
                            <p className="text-sm">Try splitting into multiple rooms or reducing guest count.</p>
                        </div>
                    )}
                </div>
            </div>
