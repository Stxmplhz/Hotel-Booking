...
export function RoomCard({ room, hotel }: RoomCardProps) {
  ...
  const isAvailable = (roomNumber: any) => {
    if(!roomNumber.unavailableDates || roomNumber.unavailableDates.length === 0) return true;
    const roomDates = roomNumber.unavailableDates.map((d: string | Date) => new Date(d).toISOString().split('T')[0]);
    const requestDates = alldates.map((d: number) => new Date(d).toISOString().split('T')[0]);
    const isFound = roomDates.some((date: string) => requestDates.includes(date));
    return !isFound;
  };
  ...
  const handleSelectRoom = () => {
    ...
    if (!hasDates) {
      ...
      setTimeout(() => {
        ...
        if (searchBar) {
          searchBar.classList.add("ring-4", "ring-red-500", "transition-all", "duration-300");
      ...
      <div className="flex flex-col md:flex-row">
        ...
        <div className="flex-1 p-6 flex flex-col">
          <div className="flex-1">
            {/* Room Title */}
            <h3 className="text-2xl text-gray-900 dark:text-white mb-4">{room.title}</h3>
            ...
            {/* Facilities */}
            <div className="mb-6">
              <h4 className="text-sm text-gray-500 dark:text-gray-300 mb-3">Facilities</h4>
          ...
          <div className="flex items-center justify-between pt-4 border-t border-gray-200">
            <div>
              <div className="text-sm text-gray-500 dark:text-gray-300 mb-1">Price per night</div>
