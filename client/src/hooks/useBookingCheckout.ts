...
export const useBookingCheckout = () => {
  ...
  const nights = dates && dates[0] ? dayDifference(dates[0].endDate, dates[0].startDate) : 1;
  const roomPrice = roomData.price || 0;
  
  const subtotal = roomPrice * nights * options.room;
  const hotelTaxRate = (roomData.taxRate || 0.07) + (roomData.serviceCharge || 0.10);
  ...
  const originalPrice = (roomPrice * nights * options.room) + taxesAndFees;
